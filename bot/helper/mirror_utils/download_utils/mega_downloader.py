    ...
    def add_download(self, link, path):
        ...
        if any([STORAGE_THRESHOLD, ZIP_UNZIP_LIMIT, LEECH_LIMIT, MEGA_LIMIT]):
            ...
            if ZIP_UNZIP_LIMIT is not None and arch:
                msg3 = f'Task Cancelled, Zip/Unzip limit is {ZIP_UNZIP_LIMIT}GB.\nYour File/Folder size is {get_readable_file_size(file_size)}.'
                limit = ZIP_UNZIP_LIMIT
            if LEECH_LIMIT is not None and listener.isLeech:
                msg3 = f'Leech Cancelled, Leech limit is {LEECH_LIMIT}GB.\nYour File/Folder size is {get_readable_file_size(api.getSize(node))}.'
                limit = LEECH_LIMIT
            elif MEGA_LIMIT is not None:
                msg3 = f'Cancelled, Mega limit is {MEGA_LIMIT}GB.\nYour File/Folder size is {get_readable_file_size(file_size)}.'
                limit = MEGA_LIMIT
